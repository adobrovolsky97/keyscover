    ...
    <div class="card border rounded-2xl shadow-lg p-4">
        ...
        <div class="container">
            ...
            <div class="orders-list mt-4" v-if="isDataLoaded && data.data.length > 0">
                <div class="collapse collapse-arrow border shadow-xl mb-4 w-full"
                     v-for="(order, orderIndex) in data.data" :key="order.id">
                    <input type="checkbox"/>
                    <div
                    ...
                    <span
                        v-if="order.payment_type === 'online'">Посилання для онлайн оплати картою (+1,5% комісії)</span>
                            ...
                            <span v-if="order.city_name"><span class="font-bold">Місто</span>: {{
                                    order.city_name
                                }}</span>
                            <span v-if="order.warehouse_name"><span
                                class="font-bold">Відділення</span>: {{ order.warehouse_name }}</span>
                            <span><span class="font-bold">Вартість</span>: {{
                                    order.total_price_usd
                                }} $ / {{ order.total_price_uah }} грн.</span>
                            <span v-if="order.discount_percent > 0"><span
                                class="font-bold">Знижка ({{ order.discount_percent }}%)</span>: {{
                                    order.discount_usd
                                }} $ / {{ order.discount_uah }} грн.</span>
                            <span v-if="order.discount_percent > 0"><span class="font-bold">Вартість зі знижкою</span>: {{
                                    order.total_with_discount_usd
                                }} $ / {{ order.total_with_discount_uah }} грн.</span>
                            <span v-if="order.comment" class="whitespace-pre-wrap"><span
                                class="font-bold">Коментар</span>: {{ order.comment }}</span>
                        </div>
                        <div class="products-list w-full overflow-x-auto">
                            <table class="table min-w-full">
                                ...
                                <tr v-for="(product, index) in order.products" :key="index">
                                    ...
                                    <td><span class="link cursor-pointer"
                                              @click="showProduct(product)">{{ product.product.name }}</span></td>
